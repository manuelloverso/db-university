...
```sql
SELECT `students`.`id` , `students`.`name` , `students`.`surname` , COUNT(`exam_student`.`student_id`) AS `attempts_for_course_exam` , `courses`.`name` AS `exam_course_name` , MAX(`exam_student`.`vote`) AS `highest_vote`
FROM `students`
